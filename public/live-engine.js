...
(function (global) {
  ...
  // shows up as a generic "Load failed"/"Failed to fetch" error. When this file runs inside
  // the Next.js app, a same-origin API route (/api/gdelt) proxies the request server-side,
  // where CORS doesn't apply at all. When running as a standalone HTML file (no server),
  // that route won't exist, so we fall back to a public CORS proxy chain as best effort.
  ...
  async function fetchMediaSentiment(placeName) {
    ...
    const [artResult, toneResult] = await Promise.allSettled([
      fetchGdeltEndpoint("artlist", { query: q, maxrecords: "10", timespan: "30d", sort: "hybridrel" }),
      fetchGdeltEndpoint("timelinetone", { query: q, timespan: "30d" }),
    ]);
